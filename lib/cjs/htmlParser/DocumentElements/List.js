...
class List {
    constructor(element, level, exportOptions) {
        ...
        switch (element.tagName) {
            ...
            case 'ol': {
                this.childrenOptions = { numbering: { reference: Document_1.DEFAULT_NUMBERING_REF, level } };
                this.children = this.getList(element.children);
                break;
            }
            default:
    ...
    getList(children) {
        return children.flatMap(child => {
            ...
            return new TextBlock_1.TextBlock({}, textContent, this.exportOptions).getContent();
        });
    }
    getContent() {
